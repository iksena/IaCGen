...
# ── I-IR Strategy ────────────────────────────────────────────────────
IIR_SYSTEM_PROMPT = """You are an expert cloud architect and IaC engineer.
You work in two explicit phases:
  Phase 1 - Produce a typed Infrastructure Intermediate Representation (I-IR)
             as a JSON object that captures resources, their dependencies, and
             their observable effects BEFORE writing any YAML.
  Phase 2 - Compile that I-IR into a valid AWS CloudFormation YAML template.

The I-IR schema:
{
  "resources": [
    {
      "id":      "<logical name, e.g. MyBucket>",
      "kind":    "<CloudFormation resource type, e.g. AWS::S3::Bucket>",
      "fields":  { "<prop>": "<value or ref>" },
      "effects": ["<tag>, e.g. public-read | encrypted | versioned"]
    }
  ],
  "edges": [
    { "type": "depends",  "from": "<id>", "to": "<id>" },
    { "type": "connects", "from": "<id>", "to": "<id>",
      "proto": "<tcp|udp|https>", "port": <number> }
  ],
  "invariants": ["<free-text constraint, e.g. encryption-at-rest required>"]
}

Wrap the I-IR in <i_ir>...</i_ir> tags.
Wrap the final CloudFormation template in <iac_template>...</iac_template> tags.
Do NOT include raw markdown code fences inside those tags."""

IIR_PLAN_TOP = """Analyse the following infrastructure requirement and produce
ONLY the I-IR JSON (Phase 1). Do not generate YAML yet.\n\nRequirement:\n"""

IIR_PLAN_BOTTOM = """\n\nOutput the I-IR inside <i_ir>...</i_ir> tags."""

IIR_GENERATE_PROMPT = """Using the I-IR you produced above, now compile it into
a complete, deployable AWS CloudFormation YAML template (Phase 2).
Rules:
- Every resource node in the I-IR must appear in the template.
- Every 'depends' edge must be expressed with DependsOn or a !Ref/!GetAtt.
- Every 'effect' must be implemented (e.g. 'encrypted' → KMSKey or SSE enabled).
- Perform a mental round-trip: re-read your YAML and verify it maps back to the I-IR.
Output the template inside <iac_template>...</iac_template> tags."""
